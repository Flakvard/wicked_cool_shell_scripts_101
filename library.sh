...
# if found and executable; 1 if not. Note that this is temporarly modifies 
...
validint()
{
  # Validates first field and test value against min value $2 and/or
  ...
  if [ ! -z $nodigits ] ; then
    echo "Invalid number format! Only digits, no commas, spaces, ect." >&2
    return 1
  fi
  ...
  if [ ! -z $max ] ; then
    # Is the input less than the maximum value?
    if [ $number -gt $max ] ; then
      echo "Your value is too big: Larges acceptable value is $max." >&2
      return 1
    fi
  fi
  return 0
